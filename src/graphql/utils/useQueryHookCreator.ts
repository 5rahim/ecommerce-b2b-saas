...
import { getData, getSingleObject } from './utils'
...
export type QueryHookCreatorOptions<T> = {
...
export function useQueryHookCreator<T>(
   ...
   query: DocumentNode,
   objectOrArray: "object" | "array",
   options: QueryHookCreatorOptions<T>,
...
export function getQueryHookReturn<T>(
   ...
   useEffect(() => {
      ...
      if (!loading && data) {
         setReturnData(objectOrArray === 'object' ? getSingleObject(data[table]) : (objectOrArray === 'array' ? getData(data[table]) : data[table + '_aggregate']))
         setIsLoading(false)
      } else if (!loading) {
         setReturnData(null)
      }
